    ...
    filtered.forEach((exp,idx)=>{
      ...
      const item=$(`
        <li class="list-group-item shadow-sm">
          <div><strong>${exp.name}</strong>
            <span class="badge bg-${colors[exp.category]} badge-category">${exp.category}</span>
      ...
      options:{
        ...
        plugins:{
          ...
          tooltip:{callbacks:{label:function(ctx){
            let label=ctx.label||'', val=ctx.parsed, total=ctx.chart._metasets[0].total;
            return `${label}: ₹${val} (${((val/total)*100).toFixed(1)}%)`;
          }}}
        },
        animation:{animateRotate:true,duration:1000,easing:'easeOutBounce'}
      }
    });
